...
两种运行方式：
  1) 在线（--live，默认）：真实调用模型（使用项目配置），token 与 cached_tokens
     取自 API 返回的 usage，成本按单价换算。需要项目根目录 .env 中配置 LLM。
...
def collect_live(keys, pricing, warmup: bool):
    ...
    try:
        ...
        print("请确保项目根目录 .env 文件中已配置 LLM 提供商和 API Key。", file=sys.stderr)
        print("或改用 --offline（离线复算，无需 key）。", file=sys.stderr)
        sys.exit(1)

    if resolved != config.MODEL:
        print(f">>> 模型名称调整：{config.MODEL} -> {resolved}")
    ...
    for k in keys:
        ...
        # KV-cache 组先跑一次「预热」，把稳定前缀写入 LLM 的 prompt cache，
...
def build_parser():
    ...
    mode.add_argument("--live", action="store_true",
                      help="在线模式（默认）：真实调用 LLM，需要项目根目录 .env 配置。")
