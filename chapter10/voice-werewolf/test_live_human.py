...
def test_strategy_audit_retains_invalid_schema_as_hard_failure(monkeypatch):
    """策略审计 fail-closed：不合规的模型评分必须硬失败，且保留原始证据。"""
    ...
    # 注入返回不合规评分的统一客户端替身
    monkeypatch.setattr(
        audit_module, "get_llm_client",
        lambda: SimpleNamespace(
            model_name="test-model",
            chat=SimpleNamespace(completions=Completion(malformed, "bad-schema")),
        ),
    )
    ...
    with pytest.raises(RuntimeError) as exc:
        evaluate_strategy(judge)
    attempts = exc.value.judge_attempts
    assert len(attempts) == 1
    assert attempts[0]["schema_valid"] is False
    assert attempts[0]["response_id"] == "bad-schema"
    # 失败尝试的溯源记录必须保持无环，附着到验收报告时才可被序列化
    json.dumps(attempts)
