...
1. **多 Agent**：每个玩家 = 一个独立 LLM Agent（统一客户端，模型由项目根目录
   .env 配置）。
...
用法（在项目根目录 ai-agant/ 下运行）：
    python chapter10/voice-werewolf/demo.py                 # 文本模式跑完整一局（LLM 决策，默认）
    python chapter10/voice-werewolf/demo.py --offline       # 离线模式：规则决策，零成本、可复现，无需 API Key
    python chapter10/voice-werewolf/demo.py --seed 7        # 换一局身份分布
    python chapter10/voice-werewolf/demo.py --players 9 --wolves 3   # 自定义人数与狼人数
    python chapter10/voice-werewolf/demo.py --voice         # 额外把公开发言合成语音到 audio/
    python chapter10/voice-werewolf/demo.py --voice --play  # 合成并播放（macOS afplay）
    python chapter10/voice-werewolf/demo.py --offline --log logs/game.log  # 完整对局日志另存一份
"""
...
from llm.client import get_llm_config  # noqa: E402 - 需先设置好 sys.path

from werewolf.game import Judge, create_players  # noqa: E402
from werewolf.roles import Faction, Role  # noqa: E402
...
def verify_simulator_trace(events):
    """在写验收报告之前校验工具/TTS/ASR 事务的完整性。

    独立校验器（validate_simulator_run.py）会对落盘证据做同样的检查；这份
    进程内副本用于防止提供商返回残缺或乱序的轨迹时（例如混入了下一回合的
    ASR），内嵌报告仍然声称自己完整。
    """
    ...
    for index, tool in ((i, e) for i, e in enumerate(events)
        ...
        if tts is None or asr is None:
            return False
        # TTS 必须发生在 ASR 之前（先合成、后识别）
        if transaction.index(tts) > transaction.index(asr):
            return False
        # 音频哈希必须前后一致：ASR 听到的就是 TTS 合成的那段音频
        ...
        if (not isinstance(audio_hash, str) or not re.fullmatch(r"[0-9a-f]{64}", audio_hash)
            ...
            return False
        # 提供商回执 ID 必须存在且全局唯一（防止拼接/重放伪造轨迹）
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--model", type=str, default=None,
                        help="覆盖 LLM 模型（默认使用项目根目录 .env 配置的 LLM_MODEL）")
    ...
    parser.add_argument("--report", default="results/acceptance_report.json",
                        help="保存回合、隐私、策略、语音事件验收报告")
    return parser


def run_game(args):
    # --model：覆盖 .env 中的 LLM_MODEL 配置，并重置全局客户端使其生效
    if args.model:
        os.environ["LLM_MODEL"] = args.model
        import werewolf.agent as agent_module
        agent_module.reset_client()
    ...
    configured_model = "（离线模式，不调用 LLM）" if args.offline else \
        (get_llm_config().get("model") or "（未在 .env 配置 LLM_MODEL）")
    print(f"模式：{mode} | 模型：{configured_model} | "
          f"种子：{args.seed} | 音频输入："
    ...
    # 验收路径的固定配置：6-8 人局且恰好 2 狼，保证结论可比较
    ...
    if not args.offline:
        ...
        try:
            ...
            # 即使所有赛后评审端点都不可用，也要保留已完成的对局与其证据。
            # 缺失的策略审计是硬性验收失败，绝不能因此丢弃报告、
            # 更不能因为模型缺席就推断验收通过。
            strategy = {
                "schema_valid": False,
                "overall_pass": False,
                "validation_errors": ["策略审计不可用"],
    ...
    # 语音边界验收：工具调用数 == 音频往返数，且轨迹事务完整、无动作漂移
...
def main():
    ...
    # 真人语音路径必须显式确认参与者知情同意，拒绝静默采集音频
    if live_human and not args.confirm_human_consent:
        print("拒绝采集音频：真人验收路径必须显式传入 --confirm-human-consent")
        sys.exit(2)
    # LLM 配置统一来自项目根目录 .env；离线模式除外
    config = get_llm_config()
    has_llm_key = bool(config.get("api_key_configured"))
    if args.simulate_user and args.simulator_speech_provider == "local":
        # local 回环依赖本机合成器，在构造会话时做存在性检查，这里不额外要求 Key
        pass
    if (args.voice or live_human) and not has_llm_key:
        print("错误：真人语音 / --voice 需要 LLM 客户端（TTS/ASR 走统一客户端的音频接口）。"
              "请先在项目根目录 .env 中配置 API_KEY / LLM_PROVIDER，"
              "或去掉 --voice 跑纯文本模式。")
        sys.exit(1)
    if not args.offline and not has_llm_key:
        print("错误：LLM 决策需要项目根目录 .env 中配置 API_KEY / LLM_PROVIDER（见项目 README）。"
