...
验证信息隔离。加 --voice 时才启用，把每条公开发言合成 mp3 存到 audio/ 目录；
在 macOS 上可用 afplay 顺带播放（--play）。客户端统一来自项目根目录 .env 配置。
"""
...
class TTS:
    """批量语音合成器：每位玩家固定一种音色，合成失败不影响游戏进行。"""
    ...
    def synth(self, speaker: str, text: str, round_no: int):
        """把一位玩家的公开发言合成 mp3（可选播放）。"""
        # 按座位号轮流分配音色，保证同一玩家音色稳定
        ...
        try:
            resp = get_client().audio.speech.create(
                model=os.getenv("OPENAI_TTS_MODEL", "tts-1"), voice=voice, input=text)
