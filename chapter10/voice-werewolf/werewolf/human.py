...
class LiveVoiceSession:
    """级联实时语音传输：VAD 端点检测 + 可选实时打断（barge-in）。

    AI 发言用所配提供商的 TTS 端点合成并立即播放；真人发言由麦克风采集，
    到句末静音为止，送往 ASR 端点识别。AI 公开播放期间麦克风可以打断播放，
    并把这段插话转成一条公开发言。建议戴耳机，避免扬声器回声被误判为打断。

    客户端统一来自项目根目录 .env 配置（get_llm_client）。
    """
    ...
    def __init__(self, out_dir: str, *, allow_interruptions: bool = True):
        ...
        # 统一客户端：API 密钥与端点均从项目根目录 .env 读取
        self.client = get_llm_client()
        self.out_dir = Path(out_dir)
        self.out_dir.mkdir(parents=True, exist_ok=True)
        self.allow_interruptions = allow_interruptions
        # 语音采集/识别相关参数（项目特定配置，均可用环境变量覆盖）
    ...
    def _write_wav(self, frames, path: Path):
        """把采集到的浮点音频帧写成一个 16bit 单声道 WAV 文件。"""
        import numpy as np
        # 合并所有音频帧、裁剪到 [-1,1]，再转成 16 位 PCM
    ...
    def say(self, speaker: str, text: str, round_no: int, *, allow_barge_in: bool = False) -> Optional[str]:
        """播放一段语音；允许打断时监听麦克风，真人开口即停并返回插话的识别文本。"""
        ...
        # 实时打断（barge-in）：播放期间持续监听麦克风，检测到语音立即停止播放，
        # 再把采集到的插话交给 ASR 作为一次打断发言。
        import numpy as np
        import sounddevice as sd
        block = 1024
    ...
    # 法官期望语音对象具有 synth() 接口（真人发言时允许被打断）
...
class HumanPlayerAgent(PlayerAgent):
    ...
    def _spoken_target(text: str, candidates: List[str], allow_none: bool) -> Optional[str]:
        """从语音识别文本中解析出玩家目标（支持编号、中文数字、英文数字等多种说法）。"""
        if allow_none and HumanPlayerAgent._explicit_none(text):
            return None
        # 匹配「P3 / player 3 / 玩家3 / 投3号」等编号说法
