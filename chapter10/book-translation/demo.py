...
实验 10-2 一键演示。
  ...
  python demo.py --model your-model    # 覆盖根目录 .env 中配置的模型
...
LLM 配置：统一使用项目根目录 ai-agant/.env（API_KEY / LLM_PROVIDER /
LLM_MODEL / BASE_URL），本脚本不读取任何项目内 .env。
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  python demo.py --no-proofreading         # 关闭审校 Agent 与修订闭环\n"
            "  python demo.py --sample-dir book --out-dir out\n"
        ),
    )
    ...
    run.add_argument(
        ...
        help="覆盖使用的模型（等价于设置根目录 .env 的 LLM_MODEL）。"
             "默认沿用项目根目录 .env 中配置的模型。",
    )
    run.add_argument(
...
def run_dry_run(args):
    ...
    # 模型名仅在 .env 已配置 LLM_MODEL 时可见；未配置也不影响离线预演
    model_label = agents.MODEL or "由项目根目录 .env 配置"
    hr(f"实验 10-2 · 离线预演（--dry-run，不调用 API，模型={model_label}）")
    ...
    print("\n离线预演结束。在项目根目录 ai-agant/.env 中配置好 LLM 后，去掉 --dry-run 即可真正运行四 Agent 协作。")


def main():
    args = parse_args()
    if args.model:
        # 必须在 import agents 之前设置：agents / llm.client 都会读取
        # LLM_MODEL 环境变量来决定使用的模型。
        os.environ["LLM_MODEL"] = args.model

    if args.dry_run:
        # 离线路径：不需要 API 密钥，也不发起任何网络调用。
        run_dry_run(args)
        return

    # 延迟导入，确保上面对 LLM_MODEL 的覆盖能在客户端创建之前生效。
    import agents
    import consistency

    # 预检统一 LLM 配置：缺 Key / 缺模型时给出清晰指引并退出
    try:
        agents.get_client()
    except Exception as e:
        print(f"错误：无法初始化统一 LLM 客户端。{e}\n"
              "请在项目根目录 ai-agant/.env 中配置 API_KEY、LLM_PROVIDER、LLM_MODEL。\n"
              "提示：想在不联网、无密钥的情况下查看四 Agent 协作结构，可运行 "
