"""双语一致性审计模块：审计翻译后的技术 Markdown 文档。

覆盖四类检查：
  - 领域术语映射（术语表内术语是否按规范译法统一出现）；
  - 代码块同步（与原文逐行一致，未被改动）；
  - LaTeX 公式语法保留；
  - 链接目标（翻译后链接是否原样保留）。
"""
...
class AuditFinding:
    ...
    category: str  # 类别："terminology" / "code_blocks" / "latex_formulas" / "link_targets"
    severity: str  # 严重程度："error" / "warning" / "info"
    message: str   # 问题描述（中文，面向审阅者）
    details: Dict[str, Any] = field(default_factory=dict)  # 结构化细节
...
class AuditReport(dict):
    """结构化审计报告：包含全部发现与一致性评分。

    同时支持字典访问（report["scores"]）与属性访问（report.scores）。
    """
    ...
    def __getattr__(self, name: str) -> Any:
        # 属性访问兜底：键不存在时抛 AttributeError 而非 KeyError
        try:
            return self[name]
        except KeyError:
            raise AttributeError(f"'AuditReport' 对象没有属性 '{name}'")
...
class BilingualConsistencyAuditor:
    ...
    def __init__(self, glossary: Optional[Dict[str, Dict[str, Dict[str, Any]]]] = None):
        """
        Args:
            glossary: 自定义双语术语表；不传则使用 DEFAULT_GLOSSARY
        """
    ...
    def _load_content(self, file_or_content: Union[str, Path]) -> str:
        ...
        if isinstance(file_or_content, str):
            ...
            try:
                ...
                pass
            # 仅当字符串“看起来像路径”且对应文件不存在时才报错；
            # 单行的 *.md 结尾字符串更可能是正文内容而不是缺失的路径。
    ...
    def _strip_code(self, text: str) -> str:
        """去掉围栏代码块与行内代码：代码按翻译指南原样保留英文，不计入术语检查。"""
    ...
    def _audit_terminology(
        ...
        评分规则：规范译法计 1.0；多种译法混用计 0.5 并告警；
        使用了非规范变体计 0.8 并提示；完全缺失计 error。
        """
        findings: List[AuditFinding] = []
        ...
        for term_en, spec in lang_glossary.items():
            ...
            # 按长度从长到短匹配各译法，避免短译法“吃掉”长译法的匹配区间
            matched_variants = []
            ...
            if not matched_variants:
                # 原文有该术语而译文完全找不到任何译法 → error
                findings.append(
                    AuditFinding(
                        category="terminology",
                        severity="error",
                        message=f"缺少领域术语 “{term_en}” 的译法，期望规范译法：“{canonical}”。",
                        details={
                            "term_en": term_en,
                ...
                findings.append(
                    AuditFinding(
                        category="terminology",
                        severity="warning",
                        message=f"术语 “{term_en}” 的译法不一致，检测到：{matched_variants}。",
                        details={
                            "term_en": term_en,
                ...
                # 用了合法但非规范的变体 → info，计 0.8 分
                findings.append(
                    AuditFinding(
                        category="terminology",
                        severity="info",
                        message=f"术语 “{term_en}” 译为了非规范变体 “{matched_variants[0]}”，规范译法是 “{canonical}”。",
                        details={
                            "term_en": term_en,
    ...
    def _audit_code_blocks(
        ...
        """检查代码块是否与源文同步（数量、语言标注、逐行内容均不得改变）。"""
        findings: List[AuditFinding] = []
        ...
        if len(source_blocks) != len(target_blocks):
            findings.append(
                AuditFinding(
                    ...
                    severity="error",
                    message=f"代码块数量不一致：源文 {len(source_blocks)} 个，译文 {len(target_blocks)} 个。",
                    details={
        ...
        if not source_blocks:
            # 译文多出的代码块已在上方产生数量不一致发现；这里按未命中计分
            ...
            if src_lang_norm != tgt_lang_norm:
                findings.append(
                    AuditFinding(
                        category="code_blocks",
                        severity="warning",
                        message=f"第 {idx + 1} 个代码块的语言标注不一致：‘{src_lang}’ vs ‘{tgt_lang}’。",
                        details={
                            "block_index": idx + 1,
            ...
            if src_lines == tgt_lines:
                ...
                findings.append(
                    AuditFinding(
                        category="code_blocks",
                        severity="error",
                        message=f"第 {idx + 1} 个代码块内容被修改或与源文失步。",
                        details={
                            "block_index": idx + 1,
    ...
    def _audit_latex_formulas(
        ...
        """检查 LaTeX 公式语法是否在译文中完整保留。"""
        findings: List[AuditFinding] = []

        source_text = self._strip_code(source_text)
        target_text = self._strip_code(target_text)

        # 只统计真正的 LaTeX 定界符 $，而非行文中的货币符号。
        # 做法：先统计下方块级 / 行内正则所消费掉的美元符，
        # 再对剩余美元符做奇偶校验来判断是否有未配对的定界符。
        ...
        # 公式相关美元符计数：每个公式消耗 2 个
        formula_dollars = (len(tgt_blocks) + len(tgt_inlines)) * 2
        # 去掉已匹配公式后剩余的美元符都是非公式用途。
        # 先剔除货币风格的 $（后面紧跟数字），如 "$5" 不是定界符。
            ...
            if formula in tgt_formula_set:
                ...
                findings.append(
                    AuditFinding(
                        category="latex_formulas",
                        severity="error",
                        message=f"LaTeX 公式缺失或被修改：‘${formula}$’。",
                ...
                for target in tgt_targets:
                    findings.append(
                        AuditFinding(
                            ...
                            severity="error",
                            message=f"译文出现了源文没有的多余链接目标：‘{target}’。",
