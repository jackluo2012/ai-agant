#!/usr/bin/env python3
"""实验 10-3：在真实浏览器使用过程中自主派生 Phone Agent。"""
...
async def main(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # 自主决策：由模型在 tool_choice=auto 下决定是否启动 Phone Agent
        ...
        # 门禁 4：WebRTC 会话真实完成（offer/answer 各一次、媒体录音齐全、双向 RTP）
        webrtc_pass = bool(
            webrtc_receipt
        ...
        # 本机麦克风路径的音频证据：TTS 与 ASR 各至少发生一次
        ...
        # 提交门禁：只有显式授权 --submit 且确实提交成功才记为通过
        ...
        # 门禁 6：隐私脱敏——轨迹中的值必须全部是 <redacted>，且不保留原始音频/转录
        persisted_trace = (
        ...
        # 实时音频门禁：ASR/TTS 调用次数必须覆盖全部字段（TTS 还包含开场与结束语）
        ...
        report = {
            ...
            "gates": {
                ...
                "real_webrtc_session": {
                    ...
                    "reason": None
                    if webrtc_pass
                    else "需要一次接通的 offer/answer 与双向 RTP 音频",
                },
        ...
        # 资源清理：尽力关闭通话通道与浏览器，正常/异常退出都保持幂等
        if (
            channel is not None
            ...
            try:
                await channel.close()
            except Exception as exc:  # noqa: BLE001 - 清理必须尽力继续
                print(f"[资源清理] 通话通道关闭失败：{type(exc).__name__}: {exc}")
