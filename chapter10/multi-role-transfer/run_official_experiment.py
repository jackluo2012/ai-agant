#!/usr/bin/env python3
"""使用原始 Moonshot/Tavily 回执和源哈希运行实验 10-1。"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", default=os.getenv("LLM_MODEL", "kimi-k3"))
...
def main(args: argparse.Namespace) -> int:
    ...
    if get_llm_client is None:
        raise RuntimeError("无法导入 LLM 客户端。请确保项目根目录已配置 llm 模块。")

    tavily_key = os.getenv("TAVILY_API_KEY", "").strip()
    if not tavily_key:
        raise RuntimeError("official run requires configured credentials: TAVILY_API_KEY")

    try:
        client = get_llm_client()
        base_url = client.base_url
    except Exception as e:
        raise RuntimeError(f"无法初始化 LLM 客户端：{e}")
