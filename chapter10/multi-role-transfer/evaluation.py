"""实验 10-1 两条路径的确定性评分辅助函数。

评分器仅评估可观察的轨迹和结果字段，从不在意隐藏的思维链。
README.md 中的协议说明了如何为实时运行添加盲评（人评或 LLM 评）和配对统计。
"""
...
def _deliverable(final_answer: str, history: list[dict]) -> str:
    ...
    优先使用传给 count_characters 的文本；否则从最终答案中提取。
...
def _contains_in_order(observed: list[str], required: list[str]) -> bool:
    """
    检查 required 是否按顺序出现在 observed 中
...
def _transition_signatures(calls: list[tuple[str, dict]]) -> list[tuple[str, str]]:
    """提取状态转换（load_skill 或 transfer_to_agent）的签名"""
    return [
...
def evaluate_task(
    ...
    *,
    ...
    预期数字故意宽松，因为实时来源可能报告略有不同的定义/舍入。
    cagr 是规范任务；coding 和 writing 为正式活动提供轻量层级。

    Args:
        final_answer: 最终答案
        history: 对话历史
        require_search: 是否要求搜索
        kind: 任务类型
        spec: 自定义任务规范
