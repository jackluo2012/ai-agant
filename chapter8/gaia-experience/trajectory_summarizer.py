...
class TrajectorySummarizer:
    ...
    async def summarize(
        ...
        try:
            ...
            summary_response = await self.llm.acompletion(
                messages=[
                    {"role": "system", "content": "你是一位分析任务执行轨迹并提取关键见解以解决未来问题的专家。"},
    ...
    def _create_summary_prompt(self, question: str, answer: str, trajectory_text: str) -> str:
        ...
        Args:
            question: 原始问题
            answer: 最终答案
            trajectory_text: 格式化的轨迹

        Returns:
            提示词字符串
        """
        prompt = f"""分析这个成功的任务执行过程，提取关键见解以解决未来类似问题。
...
请提供简洁的总结，包含以下结构：
1. 方法（APPROACH）：描述解决这个问题的高层次方法（2-3 句话）
2. 关键见解（KEY INSIGHTS）：哪些关键见解或模式使这个解决方案成功？（2-3 个要点）
3. 通用策略（GENERAL STRATEGY）：如何将此方法推广到类似问题？（1-2 句话）

以 JSON 格式返回结果，包含键："summary"、"approach"、"key_insights"（列表）、"general_strategy"
"""
        return prompt
