...
def run_gaia_task(
    ...
    instruction = """你正在解决一个 GAIA 验证问题。需要证据时使用实时网络搜索。
不要使用先验知识作为过时或冷门事实的证据。区分发现和验证，
优先使用主要来源，并遵守要求的答案格式。准备好后，仅返回 JSON 格式：
{"final_answer":"精确简洁的答案", "reason":"简短证据理由"}。"""
    tools = [SEARCH_TOOL]
...
def extract_cross_trajectory_document(records: list[dict[str, Any]], client: CampaignClient) -> tuple[ExperienceDocument, dict[str, Any]]:
    ...
    prompt = f"""比较这些经过外部评分的真实 GAIA 轨迹。提出可复用的经验，
而非任务答案。返回 JSON 格式，包含数组 `candidates`、`pitfalls`、
`exceptions`、`applies_when`。每个候选项必须包含文本和 supporting_trajectory_ids。
只有在至少两个独立的非失败轨迹支持时，才推荐来源验证、查询优化或
答案格式检查。失败轨迹可以支持误区，但绝不能支持正面建议。
