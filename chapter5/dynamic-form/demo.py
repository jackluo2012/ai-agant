...
def build_client_and_model(model_override=None):
    ...
    Raises:
        SystemExit: 当无法获取 LLM 客户端时
    """
    if get_llm_client is None:
        raise SystemExit("无法导入 LLM 客户端模块，请确保项目根目录的 llm/ 模块存在。")
...
def _extract_destination(user_request):
    """从"去XX的机票"里粗抽目的地，作为表单常量随提交一起带回。

    Args:
        user_request: 用户请求字符串

    Returns:
        str or None: 抽取的目的地，抽不到则返回 None
    """
...
def build_arg_parser():
    ...
    parser = argparse.ArgumentParser(
        ...
        "自包含 HTML 表单，用户一次提交，Agent 解析 JSON 继续任务。默认走 LLM 在线生成；"
        "加 --offline 用内置 schema 确定性渲染同样的级联表单。",
    ...
    parser.add_argument(
        "--model",
        default=None,
        help="覆盖模型名（否则使用项目配置）。--offline 下忽略此项。",
    )
    parser.add_argument(
        "--offline",
        action="store_true",
        help="离线模式：不调用 LLM，用内置机票 schema 确定性渲染级联表单。",
    )
    parser.add_argument(
        "--serve",
        action="store_true",
...
def main():
    ...
    # 离线判定：显式 --offline，或无 API_KEY 时自动回落
    offline = args.offline
    if not offline and not os.getenv("API_KEY"):
        print("未检测到 API_KEY，自动切换到离线模式（等价于 --offline）。\n")
        offline = True
    ...
    if offline:
        ...
        try:
            ...
            mode_desc = f"在线（Agent 调用 LLM，模型 {model}）"
        except Exception as e:
            print(f"警告：{e}，自动切换到离线模式。\n")
            offline = True
            mode_desc = "离线（内置 schema 确定性渲染）"
