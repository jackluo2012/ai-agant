"""一个小型的生产级 OpenAI 兼容 Agent 循环。

Creator 在模板模式下保留此循环，仅专业化
系统提示词、工具模式和领域工具实现。
"""
...
class GeneratedAgent:
    def __init__(self, *, model: str | None = None, client: Any | None = None):
        ...
        if client is not None:
            self.client = client
            self.model = model or getattr(client, 'model_name', os.getenv("OPENAI_MODEL", "gpt-4o"))
        else:
            # 尝试使用统一客户端
            if get_llm_client is not None:
                self.client = get_llm_client()
                self.model = model or getattr(self.client, 'model_name', os.getenv("LLM_MODEL", "gpt-4o"))
            else:
                # 回退到环境变量配置
                ...
                if not api_key:
                    raise RuntimeError("请设置 OPENAI_API_KEY 或 OPENROUTER_API_KEY")
