    ...
    if tools_path.exists():
        try:
            ...
            if not names or len(names) != len(set(names)):
                errors.append("tools.json 必须包含唯一的函数名称")
        except (KeyError, TypeError, json.JSONDecodeError) as exc:
            errors.append(f"无效的 tools.json：{exc}")
    ...
    if agent_tree is not None:
        ...
        for marker in ("assistant", "tool", "tool_call_id", "tool_calls"):
            if marker not in string_constants and not any(
                ...
                errors.append(f"agent 循环缺少必需的协议元素：{marker}")
        ...
        if "history" not in run_args:
            errors.append("Agent run 契约必须接受先前的多轮历史记录")
        ...
        if not (has_bound and has_bounded_loop):
            errors.append(
                "agent 循环必须暴露最大迭代次数限制并使用有界的 for/range 循环"
            )
    ...
    if main_tree is not None:
        ...
        for option in ("--task", "--model", "--history-json"):
            if option not in main_strings:
                errors.append(f"main.py 必须实现通用的实时 CLI 选项 {option}")
    ...
    for file in root.rglob("*"):
        if file.is_file() and file.name != ".env.example":
            ...
            if re.search(r"\bsk-[A-Za-z0-9_-]{12,}\b", text):
                errors.append(f"在 {file.relative_to(root)} 中可能嵌入了密钥")
...
def _run(
    ...
    # 生成的 Agents 位于 ``runs/`` 之下，而实验的仓库级
    # pytest.ini 故意从父目录发现。
    # 因此 pytest 不能可靠地将生成的 Agent 根目录添加到
    # sys.path。使被测试的可执行文件在通过 ``python main.py`` 启动时
    # 完全可导入；否则有效的 ``import agent`` 和
    # ``import domain_tools`` 语句会在单个生成的测试运行之前
    # 在收集期间失败。
