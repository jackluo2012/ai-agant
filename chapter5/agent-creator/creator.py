"""实验 5-13：创建并验证另一个 Agent 的 Agent。"""
...
class ResolvedBackend:
    """用于 creator 和生成的 Agent 的 OpenAI 兼容端点。"""

    provider: str
    client: Any  # 使用统一客户端，类型为 Any
    model: str
    api_key: str
    base_url: str | None
    ...
    def generated_agent_env(self) -> dict[str, str]:
        ...
        Scratch 模式输出是模型生成的，可能只能理解传统的
        OPENAI_* 变量。在此为两个比较分支别名一个兼容的端点，
        让它们使用相同的真实模型。
        清除 OpenRouter 可防止生成的代码仅仅因为父 shell 中存在
        过时的路由器密钥而选择它。
        """
...
def resolve_client() -> ResolvedBackend:
    ...
    优先使用项目根目录 .env 中的统一配置。
    支持 AGENT_CREATOR_PROVIDER 环境变量覆盖（用于测试）。
    """
    # 获取请求的超时时间
    request_timeout = float(os.getenv("AGENT_CREATOR_REQUEST_TIMEOUT", "600"))
    if request_timeout < 60:
        raise RuntimeError("AGENT_CREATOR_REQUEST_TIMEOUT 必须至少为 60 秒")

    # 使用统一的 LLM 客户端
    if get_llm_client is not None:
        client = get_llm_client()
        # 从客户端获取配置信息
        provider = getattr(client, 'provider', os.getenv("LLM_PROVIDER", "openai"))
        model = getattr(client, 'model_name', os.getenv("LLM_MODEL", "gpt-4o"))

        # 获取 base_url（用于生成的 agent）
        base_url = None
        if hasattr(client, 'base_url'):
            base_url = client.base_url

        # 获取 api_key（用于生成的 agent）
        # 注意：统一客户端可能不直接暴露 api_key
        # 这里我们需要从环境变量获取
    ...
    if get_llm_client is None:
        ...
        if not api_key:
            raise RuntimeError("未配置 API 密钥。请在项目根目录 .env 文件中设置 API_KEY")
    ...
    raise RuntimeError("无法解析 LLM 客户端配置")
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "你是一位资深的 Agent 工程师。仅返回一个 JSON 对象。"
                    "绝不要包含凭据。生成的代码必须完整、类型化、经过测试，"
                    "使用标准的 role/tool-call 消息，保留工具结果，限制迭代次数，"
                    "并从环境变量读取模型/API 配置。"
                ),
            },
...
"""
    ...
    def judge_live_completion(
        ...
        *,
        ...
        """判断任务完成情况，而不仅仅是进程成功退出。"""

        prompt = f"""审核生成的 Agent 的真实运行。
...
仅返回一个 JSON 对象，包含：
- passed: 布尔值
- reasoning: 简洁的基于证据的解释
- completed_requirements: 实际满足的具体需求列表
- unmet_requirements: 任何缺失、未解决、假设或仅仅向用户请求的事项列表

仅当结果真正完成了所提供的任务时才通过。一个 `ok: true` 标志、一个看似合理的
摘要或提供更多信息的邀请是不够的。拒绝不支持的假设、
缺失的工具证据、未解决的澄清请求，以及与所提供事实相反的决策。
"""
        payload, stats = self._ask(
    ...
    def judge_live_suite(
        ...
        *,
        ...
        prompt = f"""语义审核一个生成的 Agent 的所有真实运行。
...
{{
  "passed": true,
  "cases": [
    {{"id": "精确的案例 ID", "passed": true, "reasoning": "基于证据的原因",
      "unmet_requirements": []}}
  ],
  "overall_reasoning": "简洁的基于证据的结论"
}}

每个提供的案例必须恰好有一个条目。仅当最终答案达到
事实正确的决策、引用每个必需的失败及其证据、在拒绝发布时提供可操作的
补救、不让可选失败阻止批准、并在请求时使用
提供的先前轮次状态时，案例才通过。同时拒绝虚构的事实或与工具结果冲突的最终答案。
确定性检查是支持性证据，而不是橡皮图章运行的指令。
"""
