"""
Blender Python API（bpy）剪辑执行层

本模块实现 Blender 视频剪辑的核心功能，强调"代码生成"：
Proposer Agent 不去点 GUI，而是**生成一段调用 Blender Python API 的脚本**，
每个编辑操作（导入 / 裁剪 / 字幕 / 变速 / 渲染）对应一个清晰的函数调用，
再用 `blender --background --python edit.py` 无头执行。
...
def blender_available() -> bool:
    """
    检查本机是否有 blender 可执行文件

    Returns:
        是否有 blender 可执行文件（决定 backend=auto 时走 Blender 还是 ffmpeg）
    """
...
_BPY_TEMPLATE = '''"""
本文件由 blender_editor.generate_bpy_script() 自动生成。
...
def _plan_fields(plan: dict):
    ...
    Returns:
        (start, end, subtitle, slowmo) 元组

    Raises:
        ValueError: 当剪辑区间非法时抛出异常
    """
...
def render_with_blender(source: str, plan: dict, out_video: str,
    ...
    Raises:
        RuntimeError: 当 Blender 不可用或渲染失败时抛出异常
    """
