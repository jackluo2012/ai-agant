...
def main(args: argparse.Namespace) -> None:
    ...
    if args.offline:
        print("（--offline 离线模式：用预置解析器代替 LLM，无需 API Key，机制完全一致）")
    ...
    # 离线模式使用 OfflineCodeGenAgent，否则使用 CodeGenAgent
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        "自动测试 → 热加载注册 → 持久化复用）。默认使用项目配置的 LLM；"
    ...
    parser.add_argument(
        "--offline",
        action="store_true",
        help="离线模式：用预置（canned）解析器代码代替调用 LLM，无需 API Key，"
        "确定性地演示失败检测到生成到测试到热重载到持久化整条机制。",
    )
    parser.add_argument(
        "--quick",
        action="store_true",
        help="快速模式：只演示 1 种新格式（竖线分隔），跳过嵌套括号格式，省一次 LLM 调用。",
    )
    parser.add_argument(
        "--model",
        default=None,
        help="覆盖代码生成使用的模型；默认使用项目配置（.env 文件）中的模型。"
