...
class ParseError(Exception):
    ...
    当所有已注册解析器都无法解析该行时抛出，携带原始样本供 Agent 分析。
    """
...
class LogParserEngine:
    ...
    def parse_line(self, line: str) -> Dict:
        ...
        依次尝试所有解析器，成功则在结果里标注 _parser。全部失败抛 ParseError。

        Args:
            line: 一行日志字符串

        Returns:
            解析结果字典，包含 _parser 字段标注使用的解析器

        Raises:
            ParseError: 所有解析器都无法解析该行时
        """
    ...
    def load_parser_from_file(path: str) -> ParserFn:
        ...
        Args:
            path: 解析器 Python 文件路径

        Returns:
            解析函数

        Raises:
            ImportError: 无法加载模块或找不到 parse 函数时
        """
