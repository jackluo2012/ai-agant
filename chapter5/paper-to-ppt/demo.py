...
依赖：Node/Slidev（渲染）、项目根目录 .env 中的 LLM 配置。
"""
import argparse
import json
import os
...
def run_proposer_reviewer(paper_md, figures, max_rounds=MAX_ROUNDS):
    """
    运行双 Agent 方案：Proposer 生成 → Reviewer 审查 → 迭代修订

    Args:
        paper_md: 论文 Markdown 内容
        figures: 可用图表字典
        max_rounds: 最大迭代轮数

    Returns:
        包含 slides、final_pngs、history、proposer_meter、reviewer_meter 的字典
    """
...
def run_single_agent(paper_md, figures, max_rounds=MAX_ROUNDS):
    ...
    Returns:
        包含 slides、final_pngs、meter 的字典
    """
...
# 真实的 Vision 审查请用 `python demo.py`（需项目根目录 .env 配置）。
# --------------------------------------------------------------------------- #
...
def dry_run(paper_path: str):
    ...
    print("真实的 Vision 审查循环（项目根目录 .env 配置 LLM）请运行：python demo.py --mode dual --max-rounds 3")
...
def parse_args(argv=None):
    ...
    p = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  python demo.py --vision-model gpt-4o           # 覆盖视觉模型\n"
            "  python demo.py --dry-run                # 离线走通提议者-审核者循环，不调用任何 LLM\n"
            "  python demo.py --smoke                  # 仅验证 Slidev 渲染，不调用任何 LLM\n\n"
            "模型/供应商也可通过项目根目录 .env 配置；命令行 --text-model /\n"
            "--vision-model 优先级更高：API_KEY / LLM_PROVIDER / LLM_MODEL / TEXT_MODEL / VISION_MODEL"
        ),
    )
...
def main(argv=None):
    ...
    # 检查 LLM 配置（由 agents._client() 会检查）
    try:
        from llm.client import get_llm_client
        get_llm_client()
    except (ValueError, SystemExit) as e:
        print("请先在项目根目录（ai-agant/.env）中配置 LLM：")
        print("  API_KEY=your-api-key")
        print("  LLM_PROVIDER=openai (或其他提供商)")
        print("  LLM_MODEL=gpt-4o (或其他支持视觉的模型)")
        sys.exit(1)
