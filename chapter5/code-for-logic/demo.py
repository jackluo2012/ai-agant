...
try:
    ...
    DEFAULT_MODEL = _client.model_name if _client else "gpt-4o-mini"
except Exception:
...
def call_model(client, system, user, use_tools, model):
    """
    跑一轮对话(含可能的多次工具调用)
...
def run_mode(client, puzzles, mode, model):
    """
    跑一种 LLM 模式(pure/code)

    Args:
        client: LLM 客户端
        puzzles: 谜题列表
        mode: 运行模式（pure 或 code）
        model: 模型名称
    ...
    records = []
    for p in puzzles:
        text, codes = call_model(client, system, p["description"], mode == "code", model)
...
def main():
    ...
    ap.add_argument("--model", default=DEFAULT_MODEL,
                    help=f"LLM 模型名(默认 {DEFAULT_MODEL}；solver 模式忽略)")
    ...
    # solver 模式完全离线，不需要 API；其余模式需要 LLM 客户端
    ...
    if args.mode == "solver":
        ...
        client = get_client()
        if client is None:
            sys.exit("错误：pure/code/both 模式需要 LLM 配置。"
                     "请在项目根目录 .env 文件中配置 LLM 相关设置。"
                     "若只想看离线约束求解基线，请用 --mode solver。")
        print(f"模型：{model}    题目数：{len(puzzles)}    模式：{args.mode}\n")
        for m in llm_modes:
            print(f"== {LABELS[m]}({m}) ==")
            results[m] = run_mode(client, puzzles, m, model)
            print()
