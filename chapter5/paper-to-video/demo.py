...
# 默认模型/音色：使用统一 LLM 客户端配置
DEFAULT_TEXT_MODEL = "default"  # 将从统一客户端获取
DEFAULT_TTS_MODEL = "default"   # 将从统一客户端获取
...
def self_check(cfg: Config) -> int:
    ...
    # 检查统一 LLM 客户端配置
    if cfg.provider == "offline":
        print("  [OK] 供应商: offline（占位静音音轨，无需 LLM 配置）")
    else:
        # 检查项目根目录 .env 文件中的 LLM 配置
        llm_available = get_llm_client is not None
        print(f"  {'[OK]' if llm_available else '[缺失]'} LLM 客户端: {'已配置' if llm_available else '未配置'}")
        if llm_available:
            try:
                client = get_llm_client()
                print(f"  [配置] LLM 提供商: {os.getenv('LLM_PROVIDER', '未设置')}")
                print(f"  [配置] LLM 模型: {client.model_name}")
            except Exception:
                print("  [警告] LLM 客户端初始化失败，请检查项目根目录 .env 配置")
        else:
            print("  [提示] 请在项目根目录 .env 中配置 API_KEY 和 LLM_PROVIDER")

    print(f"  [配置] 供应商={cfg.provider}  讲解词模型={cfg.text_model}  "
          f"TTS 模型={cfg.tts_model}  TTS 音色={cfg.tts_voice}")
...
def main(cfg: Config) -> None:
    ...
    # 使用统一 LLM 客户端（从项目根目录 .env 读取配置）
    client = None       # 文本/讲解词客户端
    tts_client = None   # TTS 客户端
    if online:
        if get_llm_client is None:
            sys.exit("[错误] 无法导入 llm.client 模块，请确保项目根目录有 llm/client.py")

        try:
            # 获取统一客户端
            client = get_llm_client()
            tts_client = client  # TTS 使用同一客户端

            # 从统一客户端获取模型名称
            cfg.text_model = client.model_name
            cfg.tts_model = client.model_name
        except Exception as e:
            sys.exit(f"[错误] 初始化 LLM 客户端失败：{e}\n"
                     "请确保项目根目录 .env 文件中已配置 API_KEY 和 LLM_PROVIDER")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "示例：\n"
            "  python demo.py                       # 生成全部 5 页的完整讲解视频（需配置项目根目录 .env）\n"
