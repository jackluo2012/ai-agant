...
子命令：
  run     在线：Agent 生成 SQL -> 执行 -> 与参考实现比对（需 LLM 配置，默认子命令）
  gold    离线：执行内置「标准 SQL」跑 10 题 -> 与参考实现比对（无需 API，用于自检/演示）
  ask     在线：单条自然语言查询 -> 生成 SQL -> 执行并打印结果表（需 LLM 配置）
...
def run_questions(conn, employees, salaries, today, sql_provider,
    ...
    Args:
        ...
        salaries: 工资数据
        today: 当前日期
        sql_provider: SQL 提供函数，接受问题字典，返回 SQL 字符串
        qids: 只跑这些题号（None 表示全部）
        print_sql: 是否打印生成的 SQL
        max_rows: 每题结果表最多打印多少行
...
def _require_api():
    ...
    if not api_key:
        print("请先在项目根目录的 .env 文件中配置 API_KEY。")
...
def cmd_run(args):
    ...
    model = args.model or os.environ.get("LLM_MODEL", MODEL)
    ...
    def add_common(sp, with_model=False):
        ...
        if with_model:
            sp.add_argument("--model", default=None,
                            help=f"覆盖模型（默认读取项目 .env 的 LLM_MODEL，否则 {MODEL}）")

    sp_run = sub.add_parser("run", help="在线：Agent 生成 SQL 跑 10 题并校验（需 LLM 配置）")
    add_common(sp_run, with_model=True)
    sp_run.set_defaults(func=cmd_run)

    sp_gold = sub.add_parser("gold", help="离线：执行内置标准 SQL 跑 10 题并校验（无需 LLM）")
    add_common(sp_gold, with_model=False)
    sp_gold.set_defaults(func=cmd_gold)

    sp_ask = sub.add_parser("ask", help="在线：单条自然语言查询 -> SQL -> 结果表（需 LLM 配置）")
