...
from config import ARCHETYPES_MODEL_PATH, CLUSTER_K_RANGE, CLUSTER_RANDOM_STATE
...
# --- 聚类 + 层次重要性 ------------------------------------------------------
def fit(schema, results, k_range=None, save=True, verbose=True):
    ...
    for ch in sorted(set(charges)):
        ...
        for k in k_range:
            if k >= len(idx):
                break
            km = KMeans(n_clusters=k, n_init=10, random_state=CLUSTER_RANDOM_STATE).fit(Zc)
    ...
    if save:
        ...
        with open(ARCHETYPES_MODEL_PATH, "w", encoding="utf-8") as fh:
...
def load_model():
    """加载已保存的模型"""
    with open(ARCHETYPES_MODEL_PATH, encoding="utf-8") as fh:
        return json.load(fh)
