...
from config import EXTRACTED_CACHE_PATH
...
def extract_one(fact_text, schema=None, client=None, model=None, charge=None):
    """从单条判例文本抽取 {charge, factors...}。缺失因子取 null。

    Args:
        fact_text: 判例文本
        schema: 因子 schema，默认从缓存加载
        client: LLM 客户端，默认自动创建
        model: 模型名称，默认从客户端获取
        charge: 罪名（已知时直接使用，省一次调用）

    Returns:
        抽取结果字典，包含 charge 和各因子值
    """
    schema = schema or load_schema()
    if client is None:
        if get_llm_client is None:
            raise RuntimeError("无法导入 llm.client 模块，请确保项目根目录配置正确")
...
def extract_dataset(schema, use_cache=True, verbose=True):
    ...
    cases = load_dataset()
    cache = {}
    if use_cache and os.path.exists(EXTRACTED_CACHE_PATH):
        with open(EXTRACTED_CACHE_PATH, encoding="utf-8") as fh:
            for line in fh:
                if line.strip():
                    rec = json.loads(line)
    ...
    for c in cases:
        if c["id"] in cache:
            ...
            extracted = extract_one(c["fact"], schema=schema, client=client, model=model,
                                    charge=c.get("charge"))
    ...
    with open(EXTRACTED_CACHE_PATH, "w", encoding="utf-8") as fh:
        for r in results:
