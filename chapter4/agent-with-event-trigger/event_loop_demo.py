...
def make_agent_dispatch(provider: str, model: Optional[str],
    ...
    try:
        ...
        logger.info(f"✅ LLM 客户端初始化成功（提供商: {actual_provider}，模型: {actual_model}）")
    except Exception as e:
        print(f"❌ 无法初始化 LLM 客户端: {e}")
        print(f"   请检查项目根目录 .env 文件中的配置，或改用离线演示：python event_loop_demo.py --mock")
        sys.exit(1)
    ...
    logger.info(f"✅ 真实 Agent 初始化完成（提供商: {agent.provider}，模型: {agent.model}）")
