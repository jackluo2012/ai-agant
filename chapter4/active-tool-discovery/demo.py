...
def _to_openrouter_model(model: str) -> str:
    """把常见模型名映射到 OpenRouter 命名空间（用于无 API_KEY 的兜底路径）。"""
    if not model:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--model", default=None, metavar="NAME",
                    help="对话模型名（从项目根目录 .env 读取）；离线模式下忽略。")
    ap.add_argument("--embed-model", default=None,
                    metavar="NAME", help="嵌入模型名（从项目根目录 .env 读取）；离线模式下忽略。")
...
def main():
    ...
    if args.offline:
        ...
        if get_llm_client is None:
            print("无法导入 llm.client，请确保项目根目录存在 .env 文件并配置了 LLM。")
            sys.exit(1)
        ...
        # 获取嵌入模型
        embed_model = args.embed_model if args.embed_model else "text-embedding-3-small"

        # 创建嵌入后端（复用统一客户端）
        from discovery import OpenAIEmbedder
        embedder = OpenAIEmbedder(llm_client, model=embed_model)
