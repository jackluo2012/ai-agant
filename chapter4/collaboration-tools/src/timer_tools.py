...
async def set_timer(
    ...
    Args:
        duration_seconds: 定时时长（秒）
        timer_name: 可选的定时器名称
        callback_message: 定时到期时返回的消息
        callback_data: 回调中包含的可选数据
    ...
    try:
        ...
        logger.info(f"定时器 {timer_id} 已设置，时长 {duration_seconds} 秒")
        ...
        return {
            ...
            "duration_seconds": duration_seconds,
            "expiry_time": expiry_time.isoformat(),
            "message": f"定时器已设置，时长 {duration_seconds} 秒"
        }
