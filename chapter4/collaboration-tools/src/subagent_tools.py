"""子 Agent 管理工具，用于协作工具 MCP 服务器。

实现实验 4-3 中描述的子 Agent 管理原语：

    - spawn_subagent            创建子 Agent（同步或异步）
    - send_message_to_subagent  向子 Agent 发送后续消息
    - cancel_subagent           取消运行中的子 Agent
    - get_subagent_status       检查子 Agent 状态（特别是异步子 Agent）

此处的"子 Agent"是轻量级 LLM Agent 实例，与仓库其他部分使用相同的
OpenAI SDK（参见 intelligence_tools.py / config.py）。

本实验要求子 Agent 至少支持**两种上下文传递策略**并可比较其效果。
已实现两种策略并可检查（每次都会回报实际传给子 Agent 的上下文文本与 token 数）：

    - "minimal"        只传递任务和可选的手选片段。
                       保护隐私，最经济，但可能导致子 Agent 信息不足。
    - "llm_generated"  对父轨迹 + 业务规则 + 任务额外调用一次 LLM，
                       合成紧凑、经过隐私过滤的交接上下文。最智能，
                       但多花一次 LLM 调用。
"""
